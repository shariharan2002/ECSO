...
#     sample_size=f.meta_data.n_variables // 2
#     """
...
def init_population_greedy_ranked_extensions_sampled(f, pop_size=100, top_k=5, imp_dict=None, biased_frac=0.8):
    """
    Greedy + objective-aware initialization with importance-weighted sampling to reduce evaluations.
    imp_dict biases which variables to try flipping next (high score = more likely to be tried).
    """
    sample_size = f.meta_data.n_variables // 5
    n = f.meta_data.n_variables
    population = []
    ...
    while len(population) < pop_size:
        ...
        while True:
            ...
            zero_importances = [imp_dict.get(i, 0) for i in zero_indices]
            total = sum(zero_importances)
            if total == 0 or biased_frac == 0:
                # Fall back to uniform if no useful info
                sample_indices = random.sample(
                    list(zero_indices), min(sample_size, len(zero_indices))
                )
            else:
                # Normalize importances
                probs = [w / total for w in zero_importances]
                sample_indices = np.random.choice(
                    zero_indices, size=min(sample_size, len(zero_indices)),
...
def quantile_based_initializations(f, pop_size_per_quantile=50, top_overall=100, imp_dict=None, biased_frac=0.7):
    """
    Generate solutions with fixed % of 1s (0.1% to 90%), using importance-weighted randomness.
    Uses imp_dict to bias placement of 1s toward more important variables while maintaining diversity.
    """
    n = f.meta_data.n_variables
    all_feasible = []
    ...
    # Pre-sort indices by importance
    sorted_indices = sorted(range(n), key=lambda i: imp_dict.get(i, 0), reverse=True)

    def sample_biased_ones(num_ones):
        k_biased = int(biased_frac * num_ones)
        k_random = num_ones - k_biased

        # Top-5×num_ones to inject randomness within important bits
        top_candidates = sorted_indices[:min(len(sorted_indices), 5 * num_ones)]
        biased_ones = random.sample(top_candidates, min(k_biased, len(top_candidates)))

        remaining_pool = list(set(range(n)) - set(biased_ones))
        random_ones = random.sample(remaining_pool, min(k_random, len(remaining_pool)))

        return biased_ones + random_ones

    # First pass: 0.1% to <10%
    for pct in np.arange(0.1, 10.0, 0.1):
        num_ones = max(1, int((pct / 100) * n))
        print(f"Generating for {pct:.1f}% ones --> {num_ones} bits")

        for _ in range(pop_size_per_quantile):
            x = np.zeros(n, dtype=int)
            ones_indices = sample_biased_ones(num_ones)
            x[ones_indices] = 1
...
def quantile_based_initializations_2100(f, pop_size_per_quantile=50, top_overall=100, imp_dict=None, biased_frac=0.7):
    """
    Generate solutions with fixed % of 1s (1% to 99%), using importance-weighted randomness.
    Uses imp_dict to bias placement of 1s toward more important variables while maintaining diversity.
    """
    n = f.meta_data.n_variables
    all_feasible = []
    ...
    # Pre-sort all indices by importance score
    sorted_indices = sorted(range(n), key=lambda i: imp_dict.get(i, 0), reverse=True)
    ...
    def sample_biased_ones(num_ones):
        ...
        # Add randomness within top importance bits
        top_candidates = sorted_indices[:min(len(sorted_indices), 5 * num_ones)]
        biased_ones = random.sample(top_candidates, min(k_biased, len(top_candidates)))
...
def quantile_based_initializations_2200(f, pop_size_per_quantile=50, top_overall=100, imp_dict=None, biased_frac=0.7):
    """
    Generate solutions with fixed % of 1s (0.01% to 90%), using importance-weighted randomness.
    Uses imp_dict to bias placement of 1s toward more important variables while maintaining diversity.
    """
    n = f.meta_data.n_variables
    all_feasible = []
    ...
    def sample_biased_ones(num_ones):
        ...
        # Take from top 5×num_ones to add randomness within top important bits
        top_candidates = sorted_indices[:min(len(sorted_indices), 5 * num_ones)]
        biased_ones = random.sample(top_candidates, min(k_biased, len(top_candidates)))

        # Fill the rest from non-biased pool
        remaining_pool = list(set(range(n)) - set(biased_ones))
        random_ones = random.sample(remaining_pool, min(k_random, len(remaining_pool)))

        return biased_ones + random_ones

    # First pass: very small % (0.01 to <3)
    for pct in np.arange(0.01, 3.0, 0.01):
        num_ones = max(1, int((pct / 100) * n))
        print(f"Generating for {pct:.2f}% ones --> {num_ones} bits")
        for _ in range(pop_size_per_quantile):
            x = np.zeros(n, dtype=int)
            ones_indices = sample_biased_ones(num_ones)
            x[ones_indices] = 1
    ...
    # Second pass: 3% to <10%
    for pct in np.arange(3, 10.0, 0.1):
        num_ones = max(1, int((pct / 100) * n))
        print(f"Generating for {pct:.1f}% ones --> {num_ones} bits")
        for _ in range(pop_size_per_quantile):
            x = np.zeros(n, dtype=int)
...
def evolutionary_search(f, problem_id, generations=100, pop_size=100, mutation_rate=0.1):
    try:
        imp_dict = {}
        for i in range(f.meta_data.n_variables):
            x = np.zeros(f.meta_data.n_variables, dtype=int)
            x[i] = 1
            imp_dict[i]= evaluate(f, x)
        ...
        if problem_id in range(2100,2128):
            pop_size = 10
            pop_size_per_quantile = 10
            pop1 = init_population_greedy_ranked_extensions_sampled(f, 5, top_k=5, imp_dict=imp_dict, biased_frac=0.6)
            pop2 = quantile_based_initializations_2100(f, pop_size_per_quantile=pop_size_per_quantile, top_overall=pop_size, imp_dict=imp_dict, biased_frac=0.6)
            ...
            pop_size = 10
            population = quantile_based_initializations_2200(f, pop_size_per_quantile, top_overall=pop_size, imp_dict=imp_dict, biased_frac=0.6)
        else:
            pop_size = 10
            pop_size_per_quantile = 10
            population = quantile_based_initializations(f, pop_size_per_quantile=pop_size_per_quantile, top_overall=pop_size, imp_dict=imp_dict, biased_frac=0.6)
        ...
        for gen in range(generations):
            ...
            if len(last_objectives) == 10:
                ...
                if most_common and most_common[0][1] >= stagnation_threshold * 10:
                    ...
                    if problem_id in range(2100, 2128):
                        pop1  = init_population_greedy_ranked_extensions_sampled(f, 5, top_k=5, imp_dict=imp_dict, biased_frac=0.6)
                        pop2 =  quantile_based_initializations_2100(f, pop_size_per_quantile=pop_size_per_quantile, top_overall=pop_size, imp_dict=imp_dict, biased_frac=random.uniform(0.7, 0.9))
                        all_candidates = pop1 + pop2
                    elif problem_id in range(2200, 2225):
                        population = quantile_based_initializations_2200(f, pop_size_per_quantile, top_overall=pop_size, imp_dict=imp_dict, biased_frac=random.uniform(0.7, 0.9))
                    else:
                        all_candidates = quantile_based_initializations(f, pop_size_per_quantile=pop_size_per_quantile, top_overall=pop_size, imp_dict=imp_dict, biased_frac=random.uniform(0.7, 0.9))
